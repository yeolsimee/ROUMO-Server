...
public class CategoryController {
    ...
    public ResponseEntity<?> updateCategoryOrder(@RequestBody UpdateCategoryOrderRequest updateCategoryOrderRequest, @AuthenticationPrincipal User user){
        return ResponseEntity.ok(responseService.getSingleResult(categoryService.updateCategoryOrder(user.getId(), updateCategoryOrderRequest.getCategoryId(), updateCategoryOrderRequest.getTargetCategoryId())));
    }
}
