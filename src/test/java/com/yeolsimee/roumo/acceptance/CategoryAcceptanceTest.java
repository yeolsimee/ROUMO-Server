...
public class CategoryAcceptanceTest extends AcceptanceTest {
	...
	void updateCategoryOrder() {
		...
		//when
		ExtractableResponse<Response> response = 카테고리_순서_변경(UID, Long.parseLong(categoryId1), Long.parseLong(categoryId2));

		//then
		assertAll(
				() -> assertThat(response.statusCode()).isEqualTo(HttpStatus.OK.value()),
				() -> assertThat(response.jsonPath().getLong("data.firstCategory.categoryId")).isEqualTo(1),
				() -> assertThat(response.jsonPath().getLong("data.firstCategory.categoryOrder")).isEqualTo(2),
				() -> assertThat(response.jsonPath().getLong("data.secondCategory.categoryId")).isEqualTo(2),
				() -> assertThat(response.jsonPath().getLong("data.secondCategory.categoryOrder")).isEqualTo(1)
		);
	}
